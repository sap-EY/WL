...
5. Move into **Phase 8** (registered-user journey + consent). Phase 7's `RegistrationJourneyHandler` transitions completed users into `REGISTRATION_COMPLETED`; the next inbound routes via `RoutingCase.C_TRIGGER_CONSENT` (profile complete + not onboarded) into `JourneyType.REGISTERED` where Phase 8 will own the consent template send, accept/decline buttons, ice-breaker, GenAI hook, and hotline fallback.
